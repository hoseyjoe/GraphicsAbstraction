...
var FlashAbstractionMain = function() {
	var s = new com_realitycharting_flash_Sprite();
	var c;
	var d;
	var previous = s;
	var txt;
	...
	s.render(canvas,0,0);
	s.renderSvg(svg,0,0);
	console.log(s.get_x() + " " + s.get_y() + " " + s.get_width() + "," + s.get_height());
};
FlashAbstractionMain.__name__ = true;
FlashAbstractionMain.main = function() {
	new FlashAbstractionMain();
};
FlashAbstractionMain.prototype = {
	__class__: FlashAbstractionMain
};
	...
	,render: function(canvas,x,y) {
		if(canvas != null) {
			...
			while(_g < _g1.length) {
				...
				var tmp1 = StringTools.hex(d.color,6);
				...
				while(i < d.pointArray.length) if(d.pointArray[i] == 0) {
					...
					this.ctx.arc(d.pointArray[i + 1],d.pointArray[i + 2],d.pointArray[i + 3],0,2 * Math.PI,false);
					i += 4;
				} else if(d.pointArray[i] == 3) {
...
com_realitycharting_flash_Sprite.prototype = $extend(com_realitycharting_flash_DisplayObject.prototype,{
	...
	,renderSvg: function(svgElement,xx,yy) {
		...
		this.graphics.renderSvg(svgElement,this.get_x() + xx,this.get_y() + yy);
		...
		while(_g < _g1.length) {
			var c = _g1[_g];
			++_g;
			if(js_Boot.__instanceof(c,com_realitycharting_flash_Sprite) && c.visible) {
			...
			if(js_Boot.__instanceof(c,com_realitycharting_flash_TextField) && c.visible) {
		...
		var _g2 = 0;
		var _g11 = this.graphics.svgGraphics.svgNodes;
		while(_g2 < _g11.length) {
			var n = _g11[_g2];
			++_g2;
...
var com_realitycharting_flash_TextField = function() {
	...
	this.defaultTextFormat = new com_realitycharting_flash_TextFormat("arial",12,0);
	this.set_x(0);
	this.set_y(0);
	this.rotation = 0;
	this.visible = true;
...
com_realitycharting_flash_TextField.prototype = $extend(com_realitycharting_flash_DisplayObject.prototype,{
	...
	,render: function(canvas,xx,yy) {
		...
		var tmp1 = this.get_text();
		this._width_ = teextContext.measureText(tmp1).width;
		if(canvas == null) {
			console.log("NULL *************");
		}
		...
		if(this.background) {
			...
			var tmp2 = this.get_x() - this._width_;
			var tmp3 = this.get_y();
			context.fillRect(tmp2,tmp3,this._width_,this.defaultTextFormat.fontSize + 4);
			context.stroke();
			context.fillStyle = hold;
		}
		yy += this.get_height();
		var tmp4 = xx + this.get_x();
		var tmp5 = yy + this.get_y();
		context.translate(tmp4,tmp5);
		context.rotate(this.rotation / 180 * Math.PI);
		context.fillStyle = "#" + StringTools.hex(this.defaultTextFormat.fontColor,6);
		var tmp6 = this.get_text();
		context.fillText(tmp6,0,0);
		context.restore();
	}
	,get_width: function() {
		return this._width_;
